...
| 4.2 | Strong model for final synthesis | `backend/app/llm/router.py` (`try_strong_synthesis`), `backend/app/llm/providers.py`, `backend/app/agent/synthesize.py` | DONE + **live-verified** | Real Anthropic call captured 2026-05-01: `strong_model=claude-haiku-4-5-20251001`, real verdict text written by Haiku names the warm-hiking-vs-trailhead-access tradeoff. Provider preference per role is configurable (`STRONG_MODEL_PROVIDER=auto/anthropic/openai/none`); deterministic fallback runs unchanged when no key is set. |
| 4.3 | Token + cost logging per step | `backend/app/llm/providers.py` (`PRICE_TABLE_PER_MTOKENS`, `_cost_usd`), `backend/app/agent/synthesize.py`, `backend/app/models/agent_run.py` | DONE + **live-verified** | Real provider response captured: `tokens_in=383`, `tokens_out=242`, `cost_usd=0.001361`. Math + full breakdown in `README.md` § Per-query cost breakdown and `CODE_REVIEW_NOTES.md` § Real Anthropic provider proof. |
